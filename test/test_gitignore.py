...
"""
        ...
        # Test specific files that should be filtered
        def find_file_by_name(tree, name):
            if tree["type"] == "file" and tree["name"] == name:
                return tree
            for child in tree.get("children", []):
                result = find_file_by_name(child, name)
                if result:
                    return result
            return None

        # Files that should be filtered by gitignore
        filtered_files = ["config.json", "output.txt"]
        for filename in filtered_files:
            found_default = find_file_by_name(result_default["file_tree"], filename) is not None
            found_gitignore = find_file_by_name(result_gitignore["file_tree"], filename) is not None

            print(f"   {filename}: found_default={found_default}, found_gitignore={found_gitignore}")

            if found_default and not found_gitignore:
                print(f"   ✅ {filename} correctly filtered by gitignore")
            else:
                print(f"   ❌ {filename} filtering failed")
                return False

        # Files that should NOT be filtered
        not_filtered_files = ["main.py"]
        for filename in not_filtered_files:
            found_default = find_file_by_name(result_default["file_tree"], filename) is not None
            found_gitignore = find_file_by_name(result_gitignore["file_tree"], filename) is not None

            print(f"   {filename}: found_default={found_default}, found_gitignore={found_gitignore}")

            if found_default and found_gitignore:
                print(f"   ✅ {filename} correctly preserved")
            else:
                print(f"   ❌ {filename} was incorrectly filtered")
                return False
...
def test_backward_compatibility():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            analyzer_patterns = RepoAnalyzer(include_patterns=["*.py"], exclude_patterns=["*.txt"])
            result_patterns = analyzer_patterns.analyze_repository_structure(str(test_repo))
            print("✅ SUCCESS: RepoAnalyzer with patterns works")
        except Exception as e:
            print(f"❌ FAILURE: RepoAnalyzer with patterns failed: {e}")
            return False
