...
def test_cache_size_configuration():
    ...
    print("\nTesting cache size configuration...")

    try:
        from codewiki.cli.models.job import AnalysisOptions

        options1 = AnalysisOptions(cache_size=500)
        cache1 = LLMPromptCache(max_size=options1.cache_size)
        assert cache1.max_size == 500

        options2 = AnalysisOptions(cache_size=2000)
        cache2 = LLMPromptCache(max_size=options2.cache_size)
        assert cache2.max_size == 2000

        print("  ✓ Cache size configuration works correctly")
        return True
...
def test_default_cache_size():
    ...
    print("\nTesting default cache size...")

    try:
        from codewiki.cli.models.job import AnalysisOptions

        options = AnalysisOptions()
        assert options.cache_size == 1000

        print("  ✓ Default cache size is correct")
        return True
...
def main():
    ...
    tests = [
        ...
        ("Default Cache Size", test_default_cache_size),
    ]
