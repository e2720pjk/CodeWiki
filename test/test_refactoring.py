...
def test_analysis_options_separation():
    ...
    try:
        from codewiki.cli.models.job import DocumentationJob, AnalysisOptions, GenerationOptions
        ...
        assert hasattr(job.analysis_options, 'max_connectivity_files'), "analysis_options missing max_connectivity_files"
        assert hasattr(job.analysis_options, 'enable_parallel_processing'), "analysis_options missing enable_parallel_processing"
        ...
        assert not hasattr(job.generation_options, 'respect_gitignore'), "respect_gitignore should NOT be on generation_options"
        ...
        assert not hasattr(job.generation_options, 'max_entry_points'), "max_entry_points should NOT be on generation_options"
        assert not hasattr(job.generation_options, 'max_connectivity_files'), "max_connectivity_files should NOT be on generation_options"
        assert not hasattr(job.generation_options, 'enable_parallel_processing'), "enable_parallel_processing should NOT be on generation_options"
        assert not hasattr(job.generation_options, 'concurrency_limit'), "concurrency_limit should NOT be on generation_options"
        assert not hasattr(job.generation_options, 'enable_llm_cache'), "enable_llm_cache should NOT be on generation_options"
...
def test_serialization_backward_compatibility():
    ...
    try:
        ...
        assert set(gen_opts.keys()) == {'create_branch', 'github_pages', 'no_cache', 'custom_output'}, \
        ...
        assert job2.generation_options.create_branch == job.generation_options.create_branch, \
            "generation_options not preserved in round-trip"
        assert job2.analysis_options.max_files == job.analysis_options.max_files, \
