...
class TreeSitterCSharpAnalyzer:
    ...
    def _analyze(self):
        parser = get_thread_safe_parser("csharp")
        if parser is None:
            ...
            if containing_class:
                ...
                if len(type_identifiers) >= 2:
                    ...
                    if property_type and not self._is_primitive_type(property_type):
                        if property_type in [n.name for n in top_level_nodes.values()]:
                            callee_id = self._get_component_id(property_type)
                            if callee_id:
                                self.call_relationships.append(
                                    CallRelationship(
            ...
            if containing_class:
                ...
                if type_node:
                    ...
                    if field_type and not self._is_primitive_type(field_type):
                        if field_type in [n.name for n in top_level_nodes.values()]:
                            callee_id = self._get_component_id(field_type)
                            if callee_id:
                                self.call_relationships.append(
                                    CallRelationship(
                                        caller=containing_class_id,
                        ...
                        if child.type == "parameter":
                            ...
                            if type_node:
                                ...
                                if param_type and not self._is_primitive_type(param_type):
                                    if param_type in [n.name for n in top_level_nodes.values()]:
                                        callee_id = self._get_component_id(param_type)
                                        if callee_id:
                                            self.call_relationships.append(
                                                CallRelationship(
                                                    caller=containing_class_id,
